...
::alert{icon=👉}
Since [Nuxt 3.2.0](https://github.com/nuxt/nuxt/releases/tag/v3.2.0), `useFetch` is now integrated with `event.$fetch`, meaning **cookies and context are now passed to api requests automagically within internal requests**. This means that steps below should not be necessary anymore.
::
